    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={`${className} absolute   -bottom-6 bg-black left-1/2 w-fit whitespace-pre rounded-md border border-neutral-700  px-2 py-0.5 text-xs text-white`}
          role="tooltip"
          style={{ x: "-50%" }}
        >
          {children}
...
export default function Dock({
  ...
  return (
    <motion.div
      ...
      className="mx-2 flex max-w-full bg-amber-300 items-center"
    >
      <motion.div
        onMouseMove={({ pageX }) => {
          isHovered.set(1);
